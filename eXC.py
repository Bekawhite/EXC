...
WITH SMART EXTRACTION - Define exactly what data you want!
"""
...
def load_css():
    ...
    <style>
        ...
        /* Smart extraction specific styling */
        .smart-extraction-box {
...
class SmartExtractionEngine:
    """
    Smart extraction engine that lets users define exactly what data they want to extract
    """
    ...
    def create_extraction_rule(self, rule_name: str, description: str, 
                               patterns: Dict[str, str], 
                               filters: List[Dict] = None) -> Dict:
        """
        Create a new extraction rule
        
        patterns: Dictionary of {column_name: regex_pattern}
    ...
    def extract_from_example(self, example_text: str, column_names: List[str]) -> Dict[str, str]:
        """
        Generate extraction patterns from a user-provided example
        """
        patterns = {}
        ...
        pattern_templates = {
            ...
            'transaction_type': [
                r'\b(sent|received|paid|credited|debited|withdrawn|deposited|transfer)\b',
                r'\b(to|from)\s+([A-Z][a-z]+)'
            ],
            'reference': [
                r'([A-Z0-9]{5,20})',
                r'(Ref[^:]*:?\s*([A-Z0-9-]+))'
        ...
        for col in column_names:
            ...
            for pattern_type, patterns_list in pattern_templates.items():
                if any(keyword in col_lower for keyword in [pattern_type, pattern_type[:-1]]):
                    patterns[col] = patterns_list[0]  # Use first pattern as default
                    break
            
            # If no pattern found, use a generic capture
            if col not in patterns:
                patterns[col] = r'([^\n]+)'
        
        return patterns
    
    def apply_smart_extraction(self, text: str, rules: List[Dict]) -> List[Dict]:
        """
        Apply smart extraction rules to text
        """
        results = []
        ...
        for rule in rules:
            rule_results = self._apply_rule_to_text(text, rule)
            if rule_results:
                results.extend(rule_results)
        
        return results
    
    def _apply_rule_to_text(self, text: str, rule: Dict) -> List[Dict]:
        """
        ...
        # For simple extraction, search for each pattern independently
        if len(patterns) == 1 and list(patterns.values())[0] == r'([^\n]+)':
            # Simple line extraction
            lines = text.split('\n')
            for line in lines:
                line = line.strip()
                if line:
                    row = {list(patterns.keys())[0]: line}
            ...
            for line in text.split('\n'):
                ...
                for col_name, pattern in patterns.items():
                    ...
                    if match:
                        row[col_name] = match.group(1) if match.groups() else match.group(0)
                        matches_found += 1
                    else:
                        row[col_name] = ''
                
                # Only include if we found at least one match
                if matches_found > 0 and self._apply_filters(row, filters):
                    results.append(row)
        
        return results
    
    def _apply_filters(self, row: Dict, filters: List[Dict]) -> bool:
        """
        ...
        for filter_rule in filters:
            ...
            operator = filter_rule.get('operator', 'contains')
            ...
            if operator == 'contains':
                ...
                if not cell_value.startswith(filter_value):
                    return False
            elif operator == 'ends_with':
                if not cell_value.endswith(filter_value):
                    return False
            elif operator == 'regex':
                if not re.search(filter_value, cell_value, re.IGNORECASE):
                    return False
        
        return True
    
    def suggest_patterns_from_example(self, example_text: str, 
                                      columns_desired: List[str]) -> Dict[str, str]:
        """
        Suggest extraction patterns based on user example
        """
        suggested_patterns = {}
        ...
        for col in columns_desired:
            ...
            # Date patterns
            if any(term in col_lower for term in ['date', 'day', 'time']):
                suggested_patterns[col] = r'(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})'
            
            # Amount patterns
            elif any(term in col_lower for term in ['amount', 'ksh', 'money', 'payment']):
                suggested_patterns[col] = r'([Kk][Ss][Hh]?\s*\d+[,.]?\d*)'
            
            # Name patterns
            elif any(term in col_lower for term in ['name', 'payee', 'sender', 'recipient']):
                suggested_patterns[col] = r'([A-Z][A-Z\s]+|[A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)'
            
            # Phone patterns
            elif any(term in col_lower for term in ['phone', 'mobile', 'tel']):
                suggested_patterns[col] = r'(\+?\d{1,3}[-.\s]?\d{3,4}[-.\s]?\d{4,})'
            
            # Transaction type
            elif any(term in col_lower for term in ['type', 'direction', 'sent', 'received']):
                suggested_patterns[col] = r'\b(sent|received|paid|credited|debited)\b'
            
            # Reference/ID
            elif any(term in col_lower for term in ['ref', 'id', 'code', 'transaction']):
                suggested_patterns[col] = r'([A-Z0-9]{5,20})'
            
            # Default pattern
            else:
                suggested_patterns[col] = r'([^\n]+)'
...
def init_session_states():
    ...
    defaults = {
        ...
        'smart_extraction_engine': SmartExtractionEngine(),  # Smart extraction engine
        'smart_extraction_rules': [],  # User-defined extraction rules
        'smart_extraction_results': [],  # Results from smart extraction
        'smart_extraction_enabled': False,  # Whether smart extraction is enabled
        'smart_extraction_mode': 'Example-based',  # 'Example-based' or 'Rule-based'
        'smart_extraction_columns': [],  # Desired columns for smart extraction
        'smart_extraction_filters': [],  # Filters for smart extraction
    }
...
# ============================================================================
# SMART EXTRACTION UI COMPONENTS
# ============================================================================

def render_smart_extraction_ui():
    """
    Render the smart extraction configuration UI
    """
    st.markdown("""
    <div class="smart-extraction-box">
        <h3>🎯 Smart Extraction - Tell me exactly what you want!</h3>
        <p>Define the data you want to extract, and I'll find it in your document.</p>
    </div>
    """, unsafe_allow_html=True)
    
    # Extraction mode selection
    mode = st.radio(
        "How would you like to define your extraction?",
        options=[
            "Example-based (Show me an example, I'll find similar)",
            "Rule-based (Define patterns manually)",
            "Column-based (Just tell me what columns you want)"
        ],
        horizontal=True,
        key="smart_mode_radio"
    )
    
    st.session_state.smart_extraction_mode = mode
    
    if mode == "Example-based (Show me an example, I'll find similar)":
        render_example_based_extraction()
    elif mode == "Rule-based (Define patterns manually)":
        render_rule_based_extraction()
    else:
        render_column_based_extraction()
...
def render_example_based_extraction():
    """
    Render UI for example-based extraction
    """
    st.markdown("""
    <div class="pattern-example">
        <strong>📝 Example:</strong> "RHL5P339Z5 Confirmed. Ksh5,000.00 sent to ANDREW RUBIA 0705883665 on 21/8/23"
    </div>
    """, unsafe_allow_html=True)
    
    example_text = st.text_area(
        "Paste an example of the data you want to extract:",
        height=100,
        placeholder="e.g., RHL5P339Z5 Confirmed. Ksh5,000.00 sent to ANDREW RUBIA 0705883665 on 21/8/23",
        key="example_text"
    )
    
    st.markdown("**What columns do you want in your table?**")
    
    col1, col2, col3 = st.columns(3)
    
    with col1:
        date_col = st.text_input("Date column name", value="Date", key="date_col")
        date_enabled = st.checkbox("Include Date", value=True, key="date_enabled")
    
    with col2:
        payee_col = st.text_input("Payee column name", value="Payee", key="payee_col")
        payee_enabled = st.checkbox("Include Payee", value=True, key="payee_enabled")
    
    with col3:
        amount_col = st.text_input("Amount column name", value="Amount", key="amount_col")
        amount_enabled = st.checkbox("Include Amount", value=True, key="amount_enabled")
    
    col1, col2, col3 = st.columns(3)
    
    with col1:
        type_col = st.text_input("Transaction Type column", value="Type", key="type_col")
        type_enabled = st.checkbox("Include Type", value=True, key="type_enabled")
    
    with col2:
        phone_col = st.text_input("Phone column", value="Phone", key="phone_col")
        phone_enabled = st.checkbox("Include Phone", value=True, key="phone_enabled")
    
    with col3:
        ref_col = st.text_input("Reference column", value="Reference", key="ref_col")
        ref_enabled = st.checkbox("Include Reference", value=True, key="ref_enabled")
    
    # Additional custom columns
    with st.expander("➕ Add Custom Columns"):
        custom_cols = []
        num_custom = st.number_input("Number of custom columns", 0, 10, 0, key="num_custom")
        
        for i in range(int(num_custom)):
            col_name = st.text_input(f"Custom column {i+1} name", key=f"custom_col_{i}")
            col_pattern = st.text_input(f"Pattern for {col_name} (regex)", 
                                       placeholder="e.g., (\\d+)", 
                                       key=f"custom_pattern_{i}")
            if col_name and col_pattern:
                custom_cols.append({
                    'name': col_name,
                    'pattern': col_pattern
                })
    
    if st.button("Generate Extraction Rules", key="generate_rules"):
        if example_text:
            # Determine desired columns
            desired_columns = []
            patterns = {}
            
            if date_enabled:
                desired_columns.append(date_col)
                patterns[date_col] = r'(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})'
            
            if payee_enabled:
                desired_columns.append(payee_col)
                patterns[payee_col] = r'([A-Z][A-Z\s]+(?:\s+[A-Z][A-Z\s]+)*)'
            
            if amount_enabled:
                desired_columns.append(amount_col)
                patterns[amount_col] = r'([Kk][Ss][Hh]?\s*\d+[,.]?\d*)'
            
            if type_enabled:
                desired_columns.append(type_col)
                patterns[type_col] = r'\b(sent|received|paid|credited|debited)\b'
            
            if phone_enabled:
                desired_columns.append(phone_col)
                patterns[phone_col] = r'(\+?\d{1,3}[-.\s]?\d{3,4}[-.\s]?\d{4,})'
            
            if ref_enabled:
                desired_columns.append(ref_col)
                patterns[ref_col] = r'([A-Z0-9]{5,20})'
            
            # Add custom columns
            for custom in custom_cols:
                desired_columns.append(custom['name'])
                patterns[custom['name']] = custom['pattern']
            
            # Create extraction rule
            rule = st.session_state.smart_extraction_engine.create_extraction_rule(
                rule_name="Example-based extraction",
                description=f"Extracted from example: {example_text[:50]}...",
                patterns=patterns
            )
            
            st.session_state.smart_extraction_rules = [rule]
            st.session_state.smart_extraction_columns = desired_columns
            st.session_state.smart_extraction_enabled = True
            
            st.success(f"✅ Created extraction rule with {len(desired_columns)} columns!")
            
            # Show preview of what will be extracted
            st.markdown("### 🔍 Preview of extraction")
            preview_df = pd.DataFrame([{
                col: f"Will extract {col} from matching lines"
                for col in desired_columns
            }])
            st.dataframe(preview_df)

def render_rule_based_extraction():
    """
    Render UI for rule-based extraction
    """
    st.markdown("Define your own extraction patterns using regular expressions")
    
    num_rules = st.number_input("Number of columns to extract", 1, 20, 3, key="num_rules")
    ...
    for i in range(int(num_rules)):
        with st.container():
            ...
            with col1:
                col_name = st.text_input(f"Column name", key=f"rule_name_{i}")
            
            with col2:
                col_pattern = st.text_input(
                    f"Pattern (regex)", 
                    placeholder="e.g., (\\d{1,2}/\\d{1,2}/\\d{2,4})",
                    key=f"rule_pattern_{i}"
                )
    ...
    # Common pattern templates
    with st.expander("📚 Common Pattern Templates"):
        st.markdown("""
        - **Date**: `(\\d{1,2}[/-]\\d{1,2}[/-]\\d{2,4})`
        - **Amount (KSH)**: `([Kk][Ss][Hh]?\\s*\\d+[,.]?\\d*)`
        - **Phone**: `(\\+?\\d{1,3}[-.\\s]?\\d{3,4}[-.\\s]?\\d{4,})`
        - **Name**: `([A-Z][A-Z\\s]+)`
        - **Reference**: `([A-Z0-9]{5,20})`
        - **Transaction Type**: `\\b(sent|received|paid|credited|debited)\\b`
        """)
    
    if st.button("Apply Rules", key="apply_rules"):
        if rules:
            patterns = {r['name']: r['pattern'] for r in rules}
            rule = st.session_state.smart_extraction_engine.create_extraction_rule(
                rule_name="Manual rules",
                description=f"User-defined extraction with {len(rules)} columns",
                patterns=patterns
            )
            
            st.session_state.smart_extraction_rules = [rule]
            st.session_state.smart_extraction_columns = [r['name'] for r in rules]
            st.session_state.smart_extraction_enabled = True
            
            st.success(f"✅ Created {len(rules)} extraction rules!")

def render_column_based_extraction():
    """
    Render UI for simple column-based extraction
    """
    st.markdown("Just tell me what columns you want, and I'll try to find them!")
    ...
    if columns_input:
        ...
        st.markdown(f"**Will extract:** {', '.join(columns)}")
        
        if st.button("Generate Smart Extraction", key="generate_column_based"):
            # Auto-generate patterns based on column names
            patterns = {}
            for col in columns:
                col_lower = col.lower()
                
                if 'date' in col_lower:
                    patterns[col] = r'(\d{1,2}[/-]\d{1,2}[/-]\d{2,4})'
                elif any(term in col_lower for term in ['amount', 'ksh', 'money']):
                    patterns[col] = r'([Kk][Ss][Hh]?\s*\d+[,.]?\d*)'
                elif any(term in col_lower for term in ['name', 'payee']):
                    patterns[col] = r'([A-Z][A-Z\s]+)'
                elif any(term in col_lower for term in ['phone', 'mobile']):
                    patterns[col] = r'(\+?\d{1,3}[-.\s]?\d{3,4}[-.\s]?\d{4,})'
                elif any(term in col_lower for term in ['type', 'transaction']):
                    patterns[col] = r'\b(sent|received|paid|credited|debited)\b'
                elif any(term in col_lower for term in ['ref', 'id', 'code']):
                    patterns[col] = r'([A-Z0-9]{5,20})'
                else:
                    patterns[col] = r'([^\n]+)'
            
            rule = st.session_state.smart_extraction_engine.create_extraction_rule(
                rule_name="Column-based extraction",
                description=f"Extracting {len(columns)} columns",
                patterns=patterns
            )
            
            st.session_state.smart_extraction_rules = [rule]
            st.session_state.smart_extraction_columns = columns
            st.session_state.smart_extraction_enabled = True
            
            st.success(f"✅ Created extraction for {len(columns)} columns!")

def render_filter_ui():
    """
    Render UI for filtering results
    """
    filters = []
    
    num_filters = st.number_input("Number of filters", 0, 10, 0, key="num_filters")
    ...
    for i in range(int(num_filters)):
        ...
        with col1:
            filter_column = st.selectbox(
                f"Filter column {i+1}",
                options=st.session_state.smart_extraction_columns or ['Any'],
                key=f"filter_col_{i}"
            )
    ...
    if filters:
        st.session_state.smart_extraction_filters = filters

def apply_smart_extraction_to_text(text: str) -> pd.DataFrame:
    """
    Apply smart extraction to text and return DataFrame
    """
    if not st.session_state.smart_extraction_enabled or not st.session_state.smart_extraction_rules:
        return pd.DataFrame()
    
    # Add filters to rules
    rules = st.session_state.smart_extraction_rules.copy()
    for rule in rules:
        rule['filters'] = st.session_state.smart_extraction_filters
    
    # Apply extraction
    results = st.session_state.smart_extraction_engine.apply_smart_extraction(text, rules)
    ...
    if results:
        ...
        for col in df.columns:
            # Try to convert amounts to numbers
            if 'amount' in col.lower() or 'ksh' in col.lower():
                df[col] = df[col].astype(str).str.replace('Ksh', '', case=False)
                df[col] = df[col].astype(str).str.replace('ksh', '', case=False)
...
def universal_extract(text: str, page_num: int) -> List[pd.DataFrame]:
    ...
    # If smart extraction is enabled, try that first
    if st.session_state.smart_extraction_enabled:
        smart_df = apply_smart_extraction_to_text(text)
        if not smart_df.empty:
            st.session_state.extraction_method_used[f"{page_num}_0"] = 'smart_extraction'
            st.session_state.confidence_scores[f"{page_num}_0"] = 0.95
            results.append(smart_df)
            
            # Also store for preview
            st.session_state.smart_extraction_results = smart_df.to_dict('records')
            
            # Still detect content type for info
            lines = [line for line in text.split('\n') if line.strip()]
            st.session_state.content_types[page_num] = detect_content_type(lines)
...
# ============================================================================
# MODIFIED EXTRACTION SETTINGS RENDERER
# ============================================================================

def render_extraction_settings() -> Tuple[str, bool, int, int, List[int], str, bool]:
    """Render extraction settings with universal mode and smart extraction"""
    st.header("🎯 Extraction Settings")
    
    # Create tabs for different extraction modes
    tab1, tab2 = st.tabs(["📊 Standard Extraction", "🎯 Smart Extraction"])
    ...
    with tab2:
        render_smart_extraction_ui()
    
    st.markdown("---")
    
    # Page selection (common to both modes)
...
# ============================================================================
# MODIFIED DISPLAY EXTRACTION RESULTS
# ============================================================================

def display_extraction_results(total_tables_found: int, filtered_tables: Dict, tables_ignored: int, min_rows: int, min_cols: int):
    """Display extraction results with content type information and smart extraction preview"""
    if total_tables_found > 0:
        ...
        # Show smart extraction results preview if available
        if st.session_state.smart_extraction_enabled and st.session_state.smart_extraction_results:
            st.markdown("### 🎯 Smart Extraction Results Preview")
            preview_df = pd.DataFrame(st.session_state.smart_extraction_results[:10])
            if not preview_df.empty:
                st.dataframe(preview_df, use_container_width=True)
                st.caption(f"Showing first {len(preview_df)} of {len(st.session_state.smart_extraction_results)} extracted items")
        ...
        **Suggestions:**
        ...
        - Use "Smart Extraction" to define exactly what you want
    ...
    with st.sidebar:
        ...
        with st.expander("ℹ️ About", expanded=False):
            ...
            **Features:**
            ...
            - **NEW: Smart Extraction - Define exactly what you want!**
            ...
            **Version:** 4.1.0 (Smart Extraction Edition)
            """)
...
def render_welcome_screen():
    """Enhanced welcome screen for universal converter with smart extraction"""
    st.markdown("""
    <div class="info-box">
    ...
    <p><strong>NEW: Smart Extraction - Tell me exactly what you want!</strong></p>
    </div>
    """, unsafe_allow_html=True)
    
    # Feature showcase with Smart Extraction highlighted
    ...
    with col1:
        ...
        <div class="feature-card">
            ...
            <div class="feature-title">Smart Extraction (NEW!)</div>
            <div class="feature-description">Show me an example, and I'll find similar patterns. Extract exactly what you want!</div>
        </div>
    ...
    with st.expander("🎯 Try Smart Extraction - Example", expanded=True):
        st.markdown("""
        <div class="pattern-example">
            <h4>Example: Extract specific transaction details</h4>
            <p>If your document contains lines like:</p>
            <code>"RHL5P339Z5 Confirmed. Ksh5,000.00 sent to ANDREW RUBIA 0705883665 on 21/8/23"</code>
            <p>You can extract just:</p>
            <ul>
                <li><strong>Date:</strong> 21/8/23</li>
                <li><strong>Payee:</strong> ANDREW RUBIA</li>
                <li><strong>Amount:</strong> 5,000.00 (as number)</li>
                <li><strong>Type:</strong> sent (money out)</li>
                <li><strong>Phone:</strong> 0705883665</li>
                <li><strong>Reference:</strong> RHL5P339Z5</li>
            </ul>
            <p>Just go to the <strong>Smart Extraction</strong> tab and paste your example!</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        <div class="progress-step">
            ...
            <div class="step-text">Choose Extraction</div>
        </div>
        <small>Standard or Smart Extraction</small>
        """, unsafe_allow_html=True)
    ...
    with col3:
        ...
        <div class="progress-step">
            ...
            <div class="step-text">Define What You Want</div>
        </div>
        <small>Paste example or define rules</small>
        """, unsafe_allow_html=True)
    ...
    with col4:
        ...
        <small>Get exactly the data you need</small>
        """, unsafe_allow_html=True)
    ...
    if st.button("📥 Generate Excel File", type="primary", use_container_width=True):
        ...
        if not tables_to_export:
            ...
            with st.spinner(f"Creating Excel file with {total_rows:,} rows..."):
                ...
                <p><strong>Smart Extraction:</strong> {'Enabled' if st.session_state.smart_extraction_enabled else 'Disabled'}</p>
            ...
            if include_metadata:
                ...
                metadata_df = pd.DataFrame({
                    'Property': [
                        ...
                        'Data Cleaning', 'Universal Mode', 'Smart Extraction',
                        'Content Types'
                    ],
                    'Value': [
                        metadata['file_name'],
                        ...
                        "Yes" if st.session_state.smart_extraction_enabled else "No",
    ...
    st.markdown(
        ...
        <div style='text-align: center; color: #6c757d; padding: 20px;'>
            <p style='font-size: 14px;'>
                <strong>Universal Document to Excel Converter</strong> • Version 4.1.0 (Smart Extraction Edition)<br>
                ...
                <strong>NEW: Smart Extraction - Tell me exactly what you want!</strong><br>
...
def main():
    ...
    if st.session_state.pdf_uploaded and uploaded_file:
        ...
        # Get extraction settings (now includes smart extraction)
        ...
        if selected_pages and st.button("🔍 Extract Data", type="primary", use_container_width=True):
            ...
            with st.spinner(f"Analyzing {len(selected_pages)} pages..."):
                ...
                try:
                    ...
                    st.session_state.smart_extraction_results = []
                    ...
                    if debug_mode:
                        with st.expander("🔧 Debug Information"):
                            st.json({
                                ...
                                'smart_extraction_enabled': st.session_state.smart_extraction_enabled,
                                'smart_extraction_mode': st.session_state.smart_extraction_mode,
                                'smart_extraction_columns': st.session_state.smart_extraction_columns,
                                'smart_extraction_filters': st.session_state.smart_extraction_filters,
                                ...
                                'smart_extraction_results_count': len(st.session_state.smart_extraction_results),
                                'ocr_settings': {
                                    'language': ocr_lang,
